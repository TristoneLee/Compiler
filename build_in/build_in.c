...
void *__MALLOC_ARRAY_MUL(int size, int dimension, int *arraySize) {
...
void *__MALLOC_ARRAY(int size, int arraySize) {
     void *ptr;
     ptr = malloc(size * (arraySize) + INT_SIZE) + INT_SIZE;
     *(int *)(ptr - INT_SIZE) = arraySize;
     return (void *)ptr;
...
char __STRING_LESS_EQUAL(char *str1, char *str2) {
    return (strcmp(str1, str2) <= 0);
}

char __STRING_GREATER_EQUAL(char *str1, char *str2) {
    return (strcmp(str1, str2) >= 0);
}
